...
exports.allowProjectAccess = async function(
  resBuilder,
  username,
  projectID,
  accessLevel
  ...
  if (success) resBuilder.success().end();
  else resBuilder.default(400).end();
};
...
exports.verifyAccessLevel = async function(
  username,
  projectID,
  requiredAccess
) {
  logger.log(
    `Checking access level for ${username}--required=${requiredAccess}`
  );
  if (username == null) {
  ...
  try {
    ...
    if (rows[0].access_level > requiredAccess) {
      ...
      logger.log(
        `Insufficient Access--${rows[0].access_level} (${requiredAccess} required)`
      );
      return false;
    }
...
exports.getProject = async function(resBuilder, project_id) {
  var query = `SELECT * FROM projects WHERE project_id=? AND is_deleted = 0`;
  try {
    const rows = await pool.query(query, [project_id]);
    resBuilder.json["project"] = rows;
    logger.log(rows);
    return resBuilder.success().end();
  } catch (err) {
    logger.sqlErr(err);
...
exports.inviteUser = async function(
  resBuilder,
  invited,
  projectID,
  accessLevel
...
exports.resolveInvitation = async function(
  resBuilder,
  username,
  projectID,
  accepted
